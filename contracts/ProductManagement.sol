...
contract ProductManagement {
    ...
    function registerProduct(string memory lote_number, string memory product_weight, string memory creation_date, bytes32[6] memory part_array) public returns (bytes32){
        //8 
        uint i;
        ...
        //9 
        bytes32 product_hash = concatenateInfoAndHash(msg.sender, lote_number, product_weight, creation_date);
        
        require(products[product_hash].farmer == address(0), "Product ID already used");

        Product memory new_product = Product(msg.sender, lote_number, product_weight, creation_date, part_array);
        products[product_hash] = new_product;
        return product_hash;
    }

    function getBatchs(bytes32 product_hash) public returns (bytes32[6] memory){
        //10 The automatic getter does not return arrays, so lets create a function for that
